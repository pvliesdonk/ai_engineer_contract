# ENGINEERING_CONTRACT.md (AI × Peter) — v1.6.2
...
We use a **Release Automation Tool (RAT)** to generate versions, tags, release notes, and `CHANGELOG.md`. Acceptable tools include, but are not limited to, **release-please**, **semantic-release**, or equivalent.

**Requirements for the RAT:**
- Parse **Conventional Commits** to produce version bumps (SemVer), release notes, and (if used) `CHANGELOG.md`.
...
- Do **not** hand-edit `CHANGELOG.md` on `<BASE_BRANCH>`; the RAT owns release artifacts. Conflicts are resolved by favoring `main`'s generated content.

**No RAT scenario (temporary):**
- The AI will draft a Conventional-Commit–derived changelog section in the PR and update `CHANGELOG.md` as part of that PR. Once a RAT is enabled, its output becomes the source of truth.
...
- Distribution choices: copy at creation, link to canonical, or sync via a tool/subtree (avoid submodules unless strictly needed).
...
### Changelog
- v1.6.2 — **Tool-agnostic releases**: define a Release Automation Tool (RAT) interface (e.g., release-please, semantic-release), keep back-merge rule, clarify changelog ownership.
